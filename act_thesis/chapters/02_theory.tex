...
\end{equation}
He formulated that there is no viscosity for objects moving slower than $V_c$ because for an object moving at velocity $V < V_c$ supplying some momentum $p$ there will never be enough energy $\varepsilon (p)$ to create an elementary excitation. Formula \ref{eq:landau_velocity} can be displayed as the linear function with the smallest slope touching the dispersion curve (displayed orange in figure \ref{fig:dispersion_relation}).
